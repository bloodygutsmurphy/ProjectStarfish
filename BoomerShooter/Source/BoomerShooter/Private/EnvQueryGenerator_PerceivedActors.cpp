...
void UEnvQueryGenerator_PerceivedActors::GenerateItems(FEnvQueryInstance& QueryInstance) const
{
     ...
     for (AActor* ListenerActor : Listeners)
     {
         ...
         if (Listener == nullptr)
         {
             UE_LOG(LogEQS, Error, TEXT("Tried to use EnvQueryGenerator_PerceivedActors while query context actor %s doesn\'t represent a valid perception listener."),
         ...
         {
             UE_LOG(LogEQS, Error, TEXT("Tried to use EnvQueryGenerator_PerceivedActors while query context actor\'s %s UAIPerceptionComponent is missing.")
         ...
         {
             if (PerceivedActor
                 && (RadiusValue <= 0 || FVector::DistSquared(ListenerLocation, PerceivedActor->GetActorLocation()) < RadiusSquared)
     ...
     RemoveDuplicatesSorted(AllPerceivedActors);
